...
class Youcook_DataLoader(Dataset):
    ...
    def _words_to_we(self, words):
        #words = [word for word in words if word in self.we.key_to_index]#vocab]
        words = [word for word in words if word in self.we.wv.vocab]
        if words:
    ...
    def __getitem__(self, idx):
        ...
        return {'video': video, 'text': caption, 'video_id': self.data[idx]['id'],
                'audio': audio, 'nframes': nframes, 'task' : self.data[idx]['task']}
