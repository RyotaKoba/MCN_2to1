...
class Youtube_DataLoader(Dataset):
    ...
    def __init__(
        ...
        # self.count = 0
        # self.count_it=0
        # self.writer_loss=SummaryWriter(os.path.join())
    ...
    def _get_video(self, vid_path, s, e, video_id):
        ...
        for k in self.feature_path:
            feature_path[k] = os.path.join(self.feature_path[k], vid_path + ".npz")
            # print(feature_path[k])
            np_arr = np.load(feature_path[k])['arr_0']#['features']
    ...
    def __getitem__(self, idx):
        ...
        audio_path = os.path.join(self.features_path_audio, vid_path + "_spec.npz")
        ...
        audio = audio.clone().detach().to(dtype=th.float32)
        if self.random_audio_windows:
            return {'video': video, 'audio': audio, 'nframes': th.IntTensor(nframes),'video_id': video_id}
        else:
            return {'video': video, 'audio': audio, 'nframes': th.IntTensor(nframes),'video_id': video_id,
                    'text': text}#, 'time':end}
